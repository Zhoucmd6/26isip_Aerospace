...
% 任务7差异: 非常驻查询与锁定保持都走 w21.settled_q(指令就位后再测量/其实
...
f=w21.search_query(plant,qs);
s0=w21.brent_search(f,p.lower,p.upper,p.tol,min(p.maxSearchEval,n-1));
appendBrackets(s0);
...
while plant.count()<n
    ...
    for attempt=1:p.t1Retry+1
        ...
        f=w21.search_query(plant,qs);
        sr=w21.brent_search(f,a,b,p.tol,min(p.maxSearchEval,n-plant.count()));
        appendBrackets(sr);
        edge=0.05*(b-a);
