...
if ~any(strcmp(name,{'tracker','esc','spsa','bayes','qnewton','gtrack','est'}))
    error('w10:CompareBaseline','Baseline comparison needs an adaptive algorithm, got %s.',name);
end
cB=c; cB.seed=c.seed;
[logA,~]=w10.run_algorithm(name,scn,c);
mA=w10.mop_moe(logA,c);
[logB,~]=w10.run_algorithm('openloop',scn,cB);
mB=w10.mop_moe(logB,cB);
