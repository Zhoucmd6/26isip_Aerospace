...
%MAKE_PLANT 任务10风场模型库黑箱对象（速度表+功率表 + 评价日志）。
...
%   (先验曲线, 无风时标定: 空速=地速) 永远不动(只随平移调度dx/dy移动);
%   地速-功率曲线 P(v)=base_curve(|v·t̂−w|) 随风左右平移: 顺风右移、逆风左移;
%   仪表盘/速度日志显示地速。解析最优(地速口径):
%   v*(t) = q + sqrt(q² + u*² − |w|²),  q = t̂·w(顺风为正),  u* = optimum0+dx。
...
[J0min, ~] = w10.base_curve(c.optimum0, c);
rng(c.seed);
...
plant = struct('q',@q,'amendEstimate',@amend,'count',@countFcn,'table',@tbl,...
    ...
    function Jm=q(v,tag)
        assert(k<n,'w10:Plant','Evaluation budget exhausted.');
        k=k+1;
        tStep=(k-1)*c.tEval;
        ...
        for m=1:M
            ...
            [~,~,Vx,Vy]=w10.wind_field(scn,tNow,psi);
            ux=vAct*cos(psi)-Vx; uy=vAct*sin(psi)-Vy;
            u=hypot(ux,uy);
            [dxS,dyS]=w10.shift_truth(scn,tNow);
            Psub=w10.base_curve(u-dxS,c)+dyS;
        ...
        tEnd=tStep+c.tEval;
        [~,~,VxE,VyE]=w10.wind_field(scn,tEnd,psi);
        qE=VxE*cos(psi)+VyE*sin(psi);        % q=t̂·w, 顺风(沿航向)为正
        w2E=VxE^2+VyE^2;
        [dxE,~]=w10.shift_truth(scn,tEnd);
    ...
    function out=truthCurve()
        ...
        out.v=vv; out.J=w10.base_curve(vv,c);
    end
    function out=tbl()
        assert(k==n,'w10:Plant','Run incomplete: %d of %d steps.',k,n);
