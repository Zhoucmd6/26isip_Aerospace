...
%MAKE_PLANT 任务3.6风场模型库黑箱对象（速度表+功率表 + 评价日志）。
...
% 任务3.6只用static场景(无平移调度): u*固定, Pmin恒定=J0min(不随时间变化)。
...
[J0min, ~] = w36.base_curve(c.optimum0, c);
rng(c.seed);
...
plant = struct('q',@q,'amendEstimate',@amend,'count',@countFcn,'table',@tbl,...
    ...
    function Jm=q(v,tag)
        assert(k<n,'w36:Plant','Evaluation budget exhausted.');
        k=k+1;
        tStep=(k-1)*c.tEval;
        ...
        for m=1:M
            ...
            [~,~,Vx,Vy]=w36.wind_field(scn,tNow,psi);
            ux=vAct*cos(psi)-Vx; uy=vAct*sin(psi)-Vy;
            u=hypot(ux,uy);
            [dxS,dyS]=w36.shift_truth(scn,tNow);
            Psub=w36.base_curve(u-dxS,c)+dyS;
        ...
        tEnd=tStep+c.tEval;
        [~,~,VxE,VyE]=w36.wind_field(scn,tEnd,psi);
        qE=VxE*cos(psi)+VyE*sin(psi);        % q=t̂·w, 顺风(沿航向)为正
        w2E=VxE^2+VyE^2;
        [dxE,~]=w36.shift_truth(scn,tEnd);
        ustar=c.optimum0;                    % 任务3.6: 无平移调度, u*固定
    ...
    function out=truthCurve()
        ...
        out.v=vv; out.J=w36.base_curve(vv,c);
    end
    function out=tbl()
        assert(k==n,'w36:Plant','Run incomplete: %d of %d steps.',k,n);
