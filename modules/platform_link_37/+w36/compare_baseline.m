...
if ~any(strcmp(name,{'est','windinfer','sweepcal','rl','purerl'}))  % 2026-09-07精简后口径(各任务按需含purerl/sweepcal/rl)
    error('w36:CompareBaseline','Baseline comparison needs an adaptive algorithm, got %s.',name);
end
cB=c; cB.seed=c.seed;
[logA,~]=w36.run_algorithm(name,scn,c);
mA=w36.mop_moe(logA,c);
[logB,~]=w36.run_algorithm('openloop',scn,cB);
mB=w36.mop_moe(logB,cB);
