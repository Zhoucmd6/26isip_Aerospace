function info = rl_run(plant, p, n, preCal)
%RL_RUN 强化学习v3: 仿真器预训练 + 在线微调(任务3.6, sim-to-real, 纯因果, 曲线未知)。
% 任务3.7 新增可选第4参 preCal(w36.sweep_phaseA 返回值): "已标定好"模式——Stage A
% 的扫频标定改在独立试飞架次完成(不计评测预算/MOE), 评估窗直接从 Stage B/C 开始。
% 不传该参时与任务3.6行为逐位一致(标定计费)。
...
if pre
    % 任务3.7 已标定好: 世界模型整体移植自试飞架次, 本窗跳过Stage A
    coefs=preCal.coefs; wHat=preCal.wSm; fitRms=preCal.fitRms;
    uLo=preCal.uLo; uHi=preCal.uHi; uKept=preCal.uKept;
    uStar=preCal.uStar; calibSteps=0;
else
...
info=struct('best',v,'bestP',NaN,'mode','rl','muB',muB,...
    ...
    'uHat',uHat,'windEst',wEst,'phase',phHist,'windEst2',{wEst},'preCalibrated',pre);
end
