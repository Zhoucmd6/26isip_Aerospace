...
fig=uifigure('Name','任务3.7风速推断寻优：空速=地速−风速(顺风右移/逆风左移) × 风不影响运动 × 七种风场(动态演示)',...
...
header=uilabel(outer,'Text','任务3.7程序 | 空速=地速−风速(地速6+顺风3→空速3) × 曲线=空速曲线(固定) × 地速曲线顺风右移/逆风左移 × 曲线未知(调速→功率黑盒) × 首飞3→12全速域快扫标定 × MOE=纯能耗(Emin/Eactual)',...
...
algorithm=choice(g,'控制策略',1,{'sweepcal（未标定好）','sweepcal（已标定好）',...
    'hybrid（未标定好）','hybrid（已标定好）','rl（未标定好）','rl（已标定好）',...
    'purerl_on预训练在线RL','purerl_off预训练离线RL','purerl_scratch从零在线对照',...
    'openloop开环基线','windinfer已知曲线oracle参照','est已知曲线EKF(参照)',...
    'known已知风+曲线oracle上限'},...
    {'sweepcal','sweepcal_pre','hybrid','hybrid_pre','rl','rl_pre',...
    'purerl_on','purerl_off','purerl_scratch','openloop','windinfer','est','known'},...
    'sweepcal_pre');
...
logMsg('任务3.7平台对接程序就绪(数据源=平台P2物理链, 预算按秒, 就位判定交给平台; 新增: 标定类算法"已标定好"变体) |  | 主角: purerl_on预训练在线 / purerl_off预训练离线(试飞时段预训练收敛后部署, 不占评测预算) | 对照: purerl_scratch从零在线 + sweepcal/rl/hybrid(含_pre已标定好)/openloop + oracle参照');
...
prepare();
    ...
    function prepare(varargin)
        ...
        try
            buildConfig();
            % 2026-09-11 修复: 平台→本地切换后 wattScale 残留平台悬停瓦数, 本地图像
            % (含左下角功率轨迹)被放大到三四百瓦; 本地运行一律先回到本地口径。
            if ~strcmp(c.backend,'platform'), wattScale=c.pHover; end
            drawCasePreview();
            scn=w36.scenario(scenarioC.Value,c);
            if any(strcmp(algorithm.Value,{'sweepcal_pre','hybrid_pre','rl_pre'}))
                logMsg(sprintf(['已标定好模式(%s): 标定在独立试飞架次完成(不占任务窗/MOE), ',...
                    '本窗从标定完毕状态直接开始运行, MOE 只计标定后部分'],algorithm.Value));
            end
            ...
            elseif ~strcmp(c.backend,'platform')
                logMsg(sprintf('本地后端: 显示换算=本地口径(悬停 %.1f W, 按本地数据实测瓦数)',wattScale));
            end
            ...
            if any(strcmp(algorithm.Value,{'hybrid','hybrid_pre'})) && isstruct(info) && isfield(info,'uStar')
                we3=info.windEst; kF3=find(isfinite(we3(1,:)),1,'last');
                wT3x=mean(L.windX(max(1,n-30):end)); wT3y=mean(L.windY(max(1,n-30):end));
                if info.preCalibrated, ct='标定在试飞架次完成(本窗不计)'; else, ct=sprintf('首飞标定%d步',info.calibSteps); end
                logMsg(sprintf(['混合策略(3.3主角): %s(拟合RMS %.4f 归一)→在线重心转向风 | ',...
                    '风修正 ŵ=(%.2f,%.2f) 真风(末30步)=(%.2f,%.2f) | û*=%.2f(评价侧真值%.1f) | ',...
                    '探针锚谷底+低频曲线维护%d次(双向接受) | 逐步闭式调度'],...
                    ct,info.fitRms,we3(1,kF3),we3(2,kF3),wT3x,wT3y,...
                    info.uStar,c.optimum0,info.nRefit));
            end
            if any(strcmp(algorithm.Value,{'sweepcal','sweepcal_pre'})) && isstruct(info) && isfield(info,'uStar')
                we2=info.windEst; kF2=find(isfinite(we2(1,:)),1,'last');
                if info.preCalibrated, ct='已标定好(标定在试飞架次完成, 不计本窗)'; else, ct=sprintf('全速域标定%d步完成',info.calibSteps); end
                logMsg(sprintf(['曲线未知: %s(拟合RMS %.4f 归一) | ',...
                    '在线辨识 û*=%.2f m/s(评价侧真值u*=%.1f) | ŵ=(%.2f,%.2f) | ',...
                    '探针曲率b=%.4f | 之后每步闭式调度+20步联合重拟合'],...
                    ct,info.fitRms,info.uStar,c.optimum0,...
                    we2(1,kF2),we2(2,kF2),info.bEst));
            end
            if any(strcmp(algorithm.Value,{'rl','rl_pre'})) && isstruct(info) && isfield(info,'muB')
                if info.preCalibrated, ct='标定在试飞架次完成(本窗不计)'; else, ct=sprintf('标定%d步',info.calibSteps); end
                logMsg(sprintf(['RL v3(仿真器预训练+在线微调): %s→世界模型(f̂,ŵ)→',...
                    '仿真预训练60k步(零成本)→部署在线微调 | μ轮廓幅值=%.2f m/s | ',...
                    'û*=%.2f(评价侧真值%.1f) | σ=%.2f'],...
                    ct,max(info.muB)-min(info.muB),info.uStar,...
    ...
    function drawCasePreview()
        ...
        if strcmp(c.backend,'platform') && ~isempty(which('harness.make_plane_adapter'))
            ...
            wattScale=c.pHover;   % 2026-09-11 修复: 本地口径回位(平台→本地切换后残留平台瓦数)
            cv=[0.95 0.90 0.85];
